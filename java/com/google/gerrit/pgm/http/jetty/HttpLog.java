...
class HttpLog extends AbstractLifeCycle implements RequestLog {
  ...
  public void log(Request req, Response rsp) {
    ...
    map.putAll(set(P_COMMAND_STATUS, req.getHeader(GIT_COMMAND_STATUS_HEADER)));

    RequestMetricsFilter.Context ctx =
        (RequestMetricsFilter.Context) req.getAttribute(RequestMetricsFilter.METRICS_CONTEXT);
    if (ctx != null) {
      map.putAll(set(P_CPU_TOTAL, ctx.getTotalCpuTime()));
      map.putAll(set(P_CPU_USER, ctx.getUserCpuTime()));
      map.putAll(set(P_MEMORY, ctx.getAllocatedMemory()));
    }
