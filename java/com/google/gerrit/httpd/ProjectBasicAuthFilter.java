...
<<<<<<< HEAD   (9f56f3 Set version to 3.2.10-SNAPSHOT)
=======
import org.apache.commons.codec.binary.Base64;
import org.eclipse.jgit.http.server.GitSmartHttpTools;
>>>>>>> BRANCH (53649b Fix PUT/POST/DELETE REST-API with cookie authentication)
...
class ProjectBasicAuthFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (isSignedInForRequest(req) || verify(req, rsp)) {
      chain.doFilter(req, rsp);
    }
  }

  private boolean isSignedInForRequest(HttpServletRequest req) {
    WebSession currentSession = session.get();
    boolean isGitRequest = req.getRequestURI() != null && GitSmartHttpTools.isGitClient(req);
    boolean isRead = "GET".equals(req.getMethod()) || "HEAD".equals(req.getMethod());
    boolean isAlreadySignedIn = currentSession.isSignedIn();
    boolean isSignedInForRequest =
        isAlreadySignedIn
            && (isGitRequest || isRead || currentSession.isAccessPathOk(AccessPath.REST_API));
    logger.atFine().log(
        "HTTP:%s %s signedIn=%s (isAlreadySignedIn=%s, isGitRequest=%s, isRead=%s)",
        req.getMethod(),
        req.getRequestURI(),
        isSignedInForRequest,
        isAlreadySignedIn,
        isGitRequest,
        isRead);
    return isSignedInForRequest;
  }
