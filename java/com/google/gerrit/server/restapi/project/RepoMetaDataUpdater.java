...
public class RepoMetaDataUpdater {
   ...
   *
   * The method checks that user has required permissions and that user can update config without review.
   *
   ...
   *
   * The method only checks that user can update config without review and doesn't do any other
   * permissions checks. It should be used only when standard permissions checks from
  ...
  public ConfigUpdater configUpdaterWithoutPermissionsCheck(
    ...
    if(!permissionBackend
      ...
      throw new BadRequestException("Updating project config without review are disabled. Please create a change and send it for review. Some rest API methods have alternatives for creating required changes automatically - please check gerrit documentation.");
    }
    message = validateMessage(message, defaultMessage);
    MetaDataUpdate md = metaDataUpdateFactory.get().create(projectName);
    try {
