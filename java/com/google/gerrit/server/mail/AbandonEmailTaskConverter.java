...
public class AbandonEmailTaskConverter extends EmailTaskConverter {
  ...
  public OutgoingEmail convert(EmailTask emailTask) {
    ...
    if (getHeader(emailTask, EmailHeaderName.FROM_ID).isPresent()) {
      emailSender.setFrom(
          Account.id(Integer.parseInt(getHeader(emailTask, EmailHeaderName.FROM_ID).get())));
    }
    emailSender.setChangeMessage(
        emailTask.getMessage(),
        Instant.ofEpochMilli(
            Long.parseLong(getHeader(emailTask, EmailHeaderName.TIMESTAMP).get())));
    emailSender.setNotify(getNotify(emailTask.getNotifyInput()));
    emailSender.setMessageId(
        MessageId.create(getHeader(emailTask, EmailHeaderName.MESSAGE_ID).get()));
    return emailSender;
  }

  Optional<String> getHeader(EmailTask task, EmailHeaderName headerName) {
    for (EmailHeader header : task.getHeaderList()) {
